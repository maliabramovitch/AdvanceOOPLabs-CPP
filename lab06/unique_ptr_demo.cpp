...
unique_ptr<Thing> create_Thing() {
    ...
    /*
    TODO1: create a new Thing, pointed by a uniqe_ptr named p, to be returned in the end
    */
...
int main(void) {
    ...
    // Scope 2: create another Thing, give it to function that refers to it.
    {
        ...
        /*
		TODO2: check whether p still owns anything?

		*/
        if (p.release()) cout << "p is nullptr\n";
        else cout << "p is pointing to Thing: " << p.get() << "\n";
    ...
    // Scope 4: create some Things, pointed to with unique_ptr, and store them in a vector
    {
		...
		*/
        for (auto it = v.begin(); it != v.end(); ++it) {
            unique_ptr<Thing>& p = *it; // COMILATION ERROR:
